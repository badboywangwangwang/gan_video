...
@ServerEndpoint("/GanWebSocket")
@Component
public class GanWebSocket {
     ...
     * 实时弹幕注册中心
     */
    private static final Map<Integer, CopyOnWriteArraySet<GanWebSocket>> barrageRegister = new ConcurrentHashMap<>();
    ...
    public static void sendGroupMessage(String userName, Integer videoId, String message) {
        CopyOnWriteArraySet<GanWebSocket> userGroup = barrageRegister.get(videoId);
        if (userGroup != null) {
    ...
    public void onOpen(Session session) {
        ...
        try {
            ...
            // 不为空直接放入, 为空新建CopyOnWriteArraySet<GanWebSocket> svu，再放入
            if (barrageRegister.get(videoId) != null) {
                barrageRegister.get(videoId).add(this);
            } else {
                CopyOnWriteArraySet<GanWebSocket> svu = new CopyOnWriteArraySet<>();
                svu.add(this);
                barrageRegister.put(this.videoId, svu);
            }
            // 记录连接数
            GanWebSocket.addConnectCount();
            GanWebSocket.addVideoCount(videoId);
            //反馈用户
    ...
    public void onClose(Session session) {
        ...
        GanWebSocket.subConnectCount();
        GanWebSocket.subVideoCount(videoId);
        if(barrageRegister.get(videoId) != null) {
    ...
    private static synchronized void addConnectCount() {
        GanWebSocket.connectCount++;
        System.out.println("当前连接数增加了，现在连接数为" + GanWebSocket.getConnectCount());
    }
    ...
    public static synchronized void subConnectCount() {
        if (GanWebSocket.getConnectCount() > 0) {
            GanWebSocket.connectCount--;
            System.out.println("当前连接数减少了，现在连接数为" + GanWebSocket.getConnectCount());
        }
    }
    ...
    private static synchronized int getConnectCount() {
        return GanWebSocket.connectCount;
    }
